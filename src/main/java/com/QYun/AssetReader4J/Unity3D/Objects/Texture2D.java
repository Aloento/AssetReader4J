...
public class Texture2D extends Texture {
    ...
    public Texture2D(UObjectReader reader) {
        ...
        m_TextureFormat = Enums.textureFormat(reader.readInt()); //<-- Not Sure
        ...
        {
            var m_PlatformBlob = reader.readShort(); //<--reader.ReadUInt8Array()
            reader.alignStream();
        }
        ...
        if (!StringUtils.isNullOrEmpty(m_StreamData.path)){
